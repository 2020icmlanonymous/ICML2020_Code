...
def load_dataset_cora():
    ...
    A, X, z = g['A'], g['X'], g['z']
    # A - adj; X - feature matrix; z - label (0 ~ 69) ((19793,))
    ...
    next = train_size
    print(idx_train)
    validation_size = int(np.floor(19793 * 0.1))
    idx_val = idx[next: next + validation_size]
    next += validation_size

    idx_test = idx_val  # idx[next:next + 1979]
    ...
    sym_A_darray = A_darray + np.multiply(transposed_A_darray, (transposed_A_darray > A_darray)) - np.multiply(A_darray,
                                                                                                               (
                                                                                                                           transposed_A_darray > A_darray))
    A = csr_matrix(sym_A_darray)

    adj = A
    node_features = X
    return adj, node_features, y_train, y_val, y_test, train_mask, val_mask, test_mask
