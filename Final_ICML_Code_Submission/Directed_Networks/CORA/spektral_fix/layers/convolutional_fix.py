...
class FGSConv(Layer):
    ...
    def call(self, inputs):
        ...
        for k in range(self.num_comp):
            output_k = features
            for dd in range(self.num_filter):
        ...
        if len(output_dim) == 3:
            ...
            raise RuntimeError('GCN_FGS layer: wrong output dim')
    ...
    def get_pooling_weight(self, input_shape,
        ...
        pooling_kernel = self.add_weight(shape=(1, input_shape * 19793),
    ...
    # the weight variable for pooling kernel #
    def sigma_weight(self,
                     kernel_initializer=RandomUniform(minval=0.1, maxval=0.9),
                     kernel_regularizer=None,
                     kernel_constraint=min_max_norm(min_value=0.1, max_value=0.9)):
        layer = self.__class__.__name__.lower()
        gssl_sigma = self.add_weight(shape=(1,),
                                        name='sigma_GSSL_kernel' + '_' + layer + '_' + str(
                                            get_layer_uid(layer)),
                                        initializer=kernel_initializer,
                                        regularizer=kernel_regularizer,
                                        constraint=kernel_constraint,
    ...
    # the weight variable for FGS graph filter kernel #
    def beta_weight(self,
        ...
        FGS_beta = []
        for _ in range(2):
            FGS_beta.append(self.add_weight(shape=(1,),
                                             name='FGS_beta_kernel' + '_' + layer + '_' + str(
                                                 get_layer_uid(layer)),
                                             initializer=kernel_initializer,
                                             regularizer=kernel_regularizer,
                                             constraint=kernel_constraint,
    ...
    def reduce_median(self, input):
        if self.num_comp % 2 == 0:
            median_output = tf.reduce_mean(input, axis=1)
        else:
            temp_k = int(np.floor(self.num_comp / 2))
    ...
    def median_avg_pooling(self,
        ...
        temp = self.reduce_median(road_output[:, slice(0, self.num_comp)])
        temp = K.expand_dims(temp, axis=-1)
        for ii in range(1, self.channels):
            temp_inner = road_output[:, slice(ii * self.num_comp, ii * self.num_comp + self.num_comp)]
    ...
    def get_activation_weight(self, input_shape,
                           ...
                           kernel_constraint=None):
        layer = self.__class__.__name__.lower()
        activation_kernel = self.add_weight(shape=(1, input_shape * 19793),
