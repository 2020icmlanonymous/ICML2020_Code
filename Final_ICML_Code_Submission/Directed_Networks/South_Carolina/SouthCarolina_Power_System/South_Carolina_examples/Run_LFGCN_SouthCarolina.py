...
# Load data
# 1, adj processing
adj = pd.read_csv("SC_BR_B_adj_network.csv",index_col=0)
adj = adj.values
adj = csr_matrix(adj)

# 2, features
bus_input = pd.read_csv("SC_BR_B_minmax_new_feature_matrix_bus.csv",index_col = 0)
node_features = bus_input.iloc[:,0:5] # use five features
node_features = csr_matrix(node_features)

# 3, label
z = pd.read_csv("SC_BR_B_y.csv",index_col = 0).values
z = z.reshape((500,))
idx = np.arange(len(z))
label = z
...
new_scaling_weights = scaling_weights[0:int(np.ceil(500 * 2/100))]/np.sum(scaling_weights[0:int(np.ceil(500 * 2/100))])
selection_input = np.random.choice(list(range(int(np.ceil(500 * 2/100)))), int(np.ceil(500 * 1/100)), p = new_scaling_weights, replace = False)
