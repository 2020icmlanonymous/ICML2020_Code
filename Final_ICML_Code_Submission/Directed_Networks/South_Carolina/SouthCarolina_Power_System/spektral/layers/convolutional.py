...
from keras.layers import Layer, LeakyReLU, Dropout, AveragePooling2D, AveragePooling1D, MaxPooling1D, MaxPooling2D
    ...
    **Usage**
    ...
    def call(self, inputs):
        ...
        for k in range(self.ARMA_K):
            ...
            for d in range(self.ARMA_D):
                ...
                output_k = self.graph_conv_skip([output_k, features_drop, fltr],
                                                self.channels,
                                                'ARMA_skip_{}r_hid'.format(k * len(range(self.ARMA_D)) + d),
        ...
        if len(output_dim) == 3:  # [nodes, feat, 1] -> [nodes, feat_red, 1]
            output = MaxPooling1D(pool_size=self.ARMA_K, padding='same')(output)
        elif len(output_dim) == 4:  # [batch, nodes, feat, 1] -> [batch, nodes, feat_red, 1]
            output = MaxPooling2D(pool_size=(1, self.ARMA_K), padding='same')(output)
        else:
    ...
    def graph_conv_skip(self, x, channels, name,
        ...
        if recurrent_d == 0:
            ...
            kernel_1, kernel_2, bias = self.kernels_hid[recurrent_k * (len(range(self.ARMA_D))) + recurrent_d]  # authors define here to keep hidden weight parameters same -> self.kernels_hid[recurrent_k]
        features = x[0]
        features_skip = x[1]
        fltr = x[2]
...
class APPNP(Layer):
    ...
    **Usage**
    ...
    def __init__(self,
                 ...
                 activation='softmax',
                 use_bias=True,
                 kernel_initializer='glorot_uniform',  # Xavier uniform initializer
                 bias_initializer='zeros',
                 kernel_regularizer=None,
                 bias_regularizer=None,
                 activity_regularizer=None,
